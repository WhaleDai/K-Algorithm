...
public class Main {
	...
	public static void main(String[] args) throws FileNotFoundException {
        ...
        d= new ArrayList<Node>();//存储节点信息的队列
        dNew= new ArrayList<Node>();
		Reader reader= new Reader();//读取原图内容
		...
		System.out.println(end+"ms");//输出执行时间
...
/////////////////////构建匿名图/////////////////
		...
		if(totalSum % 2 != 0) 
			...
			while(totalSum>0)
			{
				...
				if(node.getDegree()>0) //判断节点是否有新增的度
				{
					int currentDegree= node.getDegree();//更新节点度数
					node.setDegree(0);
					Collections.sort(dNew, new Sort());
					for(int t=0;t<currentDegree;t++) //对节点所连接的点进行遍历
					{
						Node n= dNew.get(t);
						if(n.degree<currentDegree) break;
						else if(!n.name.equals(node.name))//若连接可能造成信息失真，则重构连接
						{
